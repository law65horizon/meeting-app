    ...
    snapshot.docChanges().forEach(async(change: any) => {
     if(change.type === 'added' || change.type === 'modified') {
      ...
      if (signalId.endsWith(`_to_${myId}`)) {
        ...
        // Handle answer
        // if (data.answer) {
        //   handleAnswer({ from: senderPeerId, sdp: data.answer.sdp, version: data.answerVersion});
        // }
...
//           // Handle answer with version check
//           if (data.answer) {
//             const answerVersion = data.answerVersion || 0; // Use a version or timestamp
//             const lastProcessedVersion = processedAnswerVersions[senderPeerId] || 0;
//             if (answerVersion > lastProcessedVersion && !processingAnswers.has(senderPeerId)) {
//               console.log('Processing new answer from:', senderPeerId, 'version:', answerVersion);
//               processingAnswers.add(senderPeerId);
//               handleAnswer({ from: senderPeerId, sdp: data.answer.sdp, version: answerVersion });
//               processingAnswers.delete(senderPeerId);
//             } else {
//               console.log('Skipping answer: already processed or being processed', {
//                 senderPeerId,
//                 answerVersion,
//                 lastProcessedVersion,
//                 isProcessing: processingAnswers.has(senderPeerId),
//               });
//             }
//           }
...
//       console.log('Connection is stable; checking if answer is still relevant');
//       // Optionally, reset the connection or log for debugging
//       // Example: Recreate the connection if needed
//       // await hangup(remotePeerId);
...
//   try {
//     console.log('Setting remote answer from:', remotePeerId, 'version:', version);
//     await pc.setRemoteDescription(new RTCSessionDescription({ type: 'answer', sdp: message.sdp }));
//     console.log('Remote answer set successfully for', remotePeerId);
//     processedAnswerVersions[remotePeerId] = version; // Update version after success
//   } catch (error) {
//     console.error('Error setting remote description:', error);
