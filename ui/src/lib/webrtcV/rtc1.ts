...
import { db, onMeetingUpdate, setMeetingOffer, setMeetingAnswer, addIceCandidate, setReady } from "./firebase";
...
export const ready = async (meetingId) => {
...
async function handleCandidate(message: any) {
  console.log('candidatemesss', message)
  const remotePeerId = message.from;
  const pc = peerConnections[remotePeerId];
  if (!pc) {
