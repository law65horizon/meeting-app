...
const MeetingRoom = () => {
  ...
  const [roomId, setRoomId] = useState<string>('main-room')
  ...
  useEffect(() => {
    ...
    socketInstance.on('connect', () => {
      ...
      socketInstance.emit('joinRoom', { roomId }, (response:any) => {
        if (response.success) {
          console.log('Joined room successfully:', roomId);
          socketInstance.emit('getRouterRtpCapabilities', {roomId}, (routerRtpCapabilities: mediasoupClient.types.RtpCapabilities) => {
            console.log('siosioi')
  ...
  useEffect(() => {
    if (socket) {
      socket.on('roomProducers', (producersInRoom: NewProducerData[]) => {
        if (!producersInRoom) return
        console.log('Received producers in room:', producersInRoom);
        producersInRoom.forEach(({ producerId }) => {
  ...
  const subscribeToProducer = async (producerId: string) => {
    if (!socket || !deviceRef.current || !consumerTransportRef.current) {
      console.error('no sockesssst', socket, deviceRef.current, consumerTransportRef.current)
      return
    };
